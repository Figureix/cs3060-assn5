	...
	for (i; i < count; i++){
		...
		if ((i + 1) < count) {
			...
			while ((nclk - clk) > timeQ && jobQ > 0) {
				...
				printf("count:%d\n", count);
				printf("164 while (runLeft[j % (i + 1)] == 0) j++;\n");
				//j++;
				while (runLeft[j % (i + 1)] == 0) j++;
			...
			while (clk < nclk && jobQ > 0){
				// get next job
				printf("189 while (runLeft[j % (i + 1)] == 0) j++;\n");
				j++;
				while (runLeft[j % (i + 1)] == 0) j++;
				pick = j; // we should have the next job OR the last job that had ran -- no new jobs added to array
				//j++;
			...
			while (jobQ > 0){
				// get next job
				printf("215 while (runLeft[j % (i + 1)] == 0) j++;\n");
				for (debug; debug < count; debug++) printf("235 (%d) runLeft[i]:%d\t finish[i]:%d\t firstRun[i]:%d\n", debug, runLeft[debug], finish[debug], firstRun[debug]);
				debug = 0;
				//j++;
				while (runLeft[j % (i + 1)] == 0) j++;
				pick = j; // we should have the next job OR the last job that had ran -- no new jobs added to array
